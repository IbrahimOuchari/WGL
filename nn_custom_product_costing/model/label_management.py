...
class LabelManagement(models.Model):
    ...
    num_lot = fields.Integer(string="Numéro de Lot", compute='_compute_num_lot')
    cumule = fields.Float(string="Cumulé", compute='_compute_cumule', digits=(16, 0))
    ...
    total_quantity_per_batch = fields.Float(string="Total Quantité par Lot",
                                            compute='_compute_total_quantity_per_batch')
    ...
    def action_create_lots(self, manufacturing_order, qty_producing):
        ...
        if not manufacturing_order.quantity_per_batch or manufacturing_order.quantity_per_batch <= 0:
        ...
        for i in range(num_lots):
            ...
            # Create the lot with minimal values, letting create() handle the name
            lot = self.create({
    ...
    def create(self, vals):
        """Override create to ensure proper sequence generation."""
        if vals.get('name', '/') == '/':
            vals['name'] = self.env['ir.sequence'].next_by_code('label.management.sequence.new') or '/'
        return super(LabelManagement, self).create(vals)
