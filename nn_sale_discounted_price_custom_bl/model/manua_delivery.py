...
class ManualDelivery(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        # Check if more than one partner is selected
        ...
        if sale_lines:
            ...
            # Convert to manual.delivery.lines, including pu_remise
            res["line_ids"] = [
                (
                    0,
                    0,
                    {
                        ...
                        "pu_remise": line.pu_remise,  # Fetch pu_remise field
                    },
                )
    ...
    def confirm(self):
        ...
        sale_order_lines.with_context(
            BMG_stock_starter=self
        )._action_launch_stock_rule_manual()


class ManualDeliveryExtended(ManualDelivery):
    _inherit = "manual.delivery"

    pu_remise = fields.Float(string='Prix Unitaire Remisé', related='line_ids.order_line_id.pu_remise', store=True)

    @api.model
    def default_get(self, fields):
        res = super(ManualDeliveryExtended, self).default_get(fields)
